...
외부 응답 `error.code`는 BE3의 세분화 코드를 사용한다.
...
- 사용자 표시 문구는 `error.message`를 그대로 사용
- 필요 시 FE는 코드별 라벨(예: "JSON 파싱 오류")을 별도로 매핑

### 3.3 JSON_PARSE_ERROR 처리

- `JSON_PARSE_ERROR`는 더 이상 1차 표준 코드로 사용하지 않는다.
- 하위 호환이 필요하면 FE 매핑에서 `PARSE_*`를 `JSON_PARSE_ERROR` 그룹 라벨로만 묶어 표시한다.
...
- answer 내 `[[CITE:n]]` 토큰과 `citations.ref_id`는 1:1 매핑
- `citations.ref_id`는 응답 내 유일
- `chunk_id`는 검색 결과 목록에 존재
- `case_id`는 해당 `chunk_id`와 일치
- `snippet`은 공백 문자열 금지
...
- 성공 응답에서 `qa_validation`은 항상 포함(필수)
- 최소 구조:
...
- `is_valid=false`인데 answer를 반환하는 경우는 Week 1에서는 금지하고 실패 응답으로 내린다.
...
- [x] answer에 `[[CITE:n]]` 토큰 삽입
- [x] citations에 ref_id/chunk_id/case_id/snippet 채움
- [x] 가능하면 doc_id, relevance_score 포함
- [x] confidence enum 준수

### BE3

- [x] PARSE_* 코드 기반 파서/재시도 유지
- [x] qa_validation 항상 생성
- [x] chunk_id/case_id/snippet 정합성 검증
- [x] 실패 시 success=false + error 객체 표준 반환

### FE

- [x] answer에서 `[[CITE:n]]` 파싱해 `[출처 n]` 배지 렌더링
- [x] 배지 hover 시 citations.ref_id == n의 snippet 표시
- [x] success=false면 error.message 배너 표시
- [x] meta 3필드 및 qa_validation.warnings 렌더링
